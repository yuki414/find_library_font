<template>
  <v-card>
    <div id="updateUser">
      <!-- ここからGoogle map -->
      <div>
        <GmapMap :center="center" :zoom="zoom" style="width: 100%; height: 300px">
          <!-- googlemapのマーカー設定箇所 -->
          <GmapInfoWindow
            :options="infoOptions"
      ...
      <v-row>
        ...
        <v-col cols="6" md="4" offset="1">
          <h2>住所登録</h2>
          <v-form ref="form">
            <v-text-field v-model="address1" lable="都道府県" required></v-text-field>

            <v-text-field v-model="address2" lable="市区町村" required></v-text-field>

            <v-text-field v-model="address3" lable="番地以降" required></v-text-field>

            <v-btn :color="color" dark class="mr-4" @click="updateUser()">更新</v-btn>
          </v-form>
        </v-col>
        <!-- ここまで住所 -->
        <!-- ここから図書館検索結果表示  -->
        <v-col cols="12" md="7">
          <v-card class="pa-2" max-width="80%" tile>
            <v-container id="scroll-target" style="max-height: 300px" class="overflow-y-auto">
              <v-row v-scroll:#scroll-target align="center" justify="center">
                <v-list float>
                  <v-subheader>
                  ...
                  <v-list-item-group v-model="lib" color="primary">
                    <v-list-item v-for="(lib, index) in libaryAll" :key="index">
  ...
  methods: {
    ...
    clickToggle(lib) {
      ...
      if (this.checked.indexOf(lib.libid) >= 0) {
        ...
        axios.delete("library_relationships", { data: params });
        // ここにmarkerを消す作業
        ...
        console.log("create");
        // ここにcreate
        axios.post("library_relationships", {
          libRel: {
            user_id: this.uid,
            library_id: lib.id
          }
