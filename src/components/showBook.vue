...
export default {
  ...
  methods: {
    ...
    toggleInfoWindow (marker) {
      ...
      this.infoContent = {
        title: marker.title,
        description: JSON.stringify(marker.status).slice(1,-1).replace(/"/g, " ")
      }
    ...
    searchBook () {
      axios.get('http://localhost:3000/api/book_search',{
        "params": {
          "user_id": this.uid,
      ...
      .then(response=>{
        ...
        if (this.continue==1) {
          ...
          for (var i=0; i<this.markers.length; i++){
            this.markers[i].status=this.result[this.markers[i].systemid].libkey
          }
